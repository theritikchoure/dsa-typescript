...
function heapify1(arr: number[], size: number, index: number): void {
    let smallest = index;
    const left = 2 * index + 1; // Calculate the left child index
    const right = 2 * index + 2; // Calculate the right child index
...
function kthElement1(arr: number[], k: number): number {
    let size = arr.length;
...
const kthArray1: number[] = [7, 10, 4, 20, 15];
const kValue1: number = 3;
